...
`, i = document.querySelector("script[data-key]"), d = i.getAttribute("data-key"), a = document.body.classList.contains("darkMode") || i.getAttribute("darkMode"), p = document.getElementsByClassName("response-rocket");
...
if (p.length) {
  ...
  for (let t = 0; t < o.length; t++)
    o[t].addEventListener("click", function() {
      const { hash: c, pathname: r } = window.location, k = this.getAttribute("data-reaction");
      n || fetch(m, {
        method: "POST",
        headers: {
        ...
        n = e.exceededClickLimit, e.reactions && e.reactions.map((h) => {
          const u = document.querySelectorAll(`.response-rocket-button[data-reaction="${h.reaction}"]`);
          for (let l = 0; l < u.length; l++)
            u[l].querySelector("span.response-rocket-count").innerHTML = h.click;
        });
      }).catch((e) => console.error(e));
    });
  let s = "https://responserocket.app/api/page";
  i.getAttribute("data-dev") && (s = "https://responserocket.test/api/page"), b(d, s);
}
